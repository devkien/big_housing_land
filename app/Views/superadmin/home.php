...
<body>
    <div class="app-container" style="background-color: #E8F4FF;">
        ...
        <section class="menu-section" id="menu-container">
            <?php
            // Try admin layout for admin role, otherwise main layout. Use safe file_exists fallback and log if missing.
            $__preferred = (($_SESSION['user']['quyen'] ?? '') === 'admin') ? (__DIR__ . '/../admin/layouts/menu.php') : (__DIR__ . '/layouts/menu.php');
            if (!file_exists($__preferred)) {
                $__alt = __DIR__ . '/layouts/menu.php';
                if (file_exists($__alt)) {
                    $__preferred = $__alt;
                } else {
                    $__alt2 = __DIR__ . '/../admin/layouts/menu.php';
                    if (file_exists($__alt2)) {
                        $__preferred = $__alt2;
                    } else {
                        error_log("Menu include not found (superadmin): preferred={$__preferred}");
                    }
                }
            }
            if (file_exists($__preferred)) require_once $__preferred;
            ?>
        </section>
