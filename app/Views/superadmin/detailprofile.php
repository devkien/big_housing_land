...
<body>
    <div class="app-container" style="background: white;">
        ...
        <header class="profile-detail-header">
            <a href="<?= BASE_URL ?>/superadmin/profile" style="color: black; font-size: 18px;"><i class="fa-solid fa-chevron-left"></i></a>
        ...
        <div class="cover-wrapper">
            ...
            <?php
            // Normalize avatar path: support absolute URL, root-relative path, or stored relative path
            ...
            if (!empty($avatar)) {
                ...
                if (stripos($trim, 'http') === 0) {
                    ...
                    // root-relative - prepend BASE_URL to be safe
                    $avatarSrc = rtrim(BASE_URL, '/') . $trim;
                } else {
                    // relative path from DB - assume it's already under uploads or full relative path
            ...
            <?php
            // Only show edit button when viewing your own profile
            $sessionUser = $sessionUser ?? ($_SESSION['user'] ?? null);
            $showEditButton = false;
            if (!empty($sessionUser) && !empty($user) && isset($sessionUser['id']) && isset($user['id']) && (int)$sessionUser['id'] === (int)$user['id']) {
                $showEditButton = true;
            }
            ?>
            <?php if ($showEditButton): ?>
                <a href="<?= BASE_URL ?>/superadmin/editprofile" class="edit-profile-btn">
                    <i class="fa-solid fa-pen"></i> Chỉnh sửa hồ sơ
                </a>
            <?php endif; ?>
            <?php
            // CCCD visibility: only super_admin or the owner can see full CCCD
            ...
            <?php // rating moved into right-column below 
            ?>
        </div>
        ...
        <div class="info-grid">
            ...
            <div class="info-list-right">
                ...
                <?php if ($canViewCCCD): ?>
                    ...
                    ?>
                    <img src="<?= htmlspecialchars($cccdSrc, ENT_QUOTES, 'UTF-8') ?>" class="id-card-img" alt="CCCD" style="background:#eee;">
